#' Create DEXSeq from feactureCounts
#'
#' This function will take featureCounts output and together with other outputs
#' will create a DEXSeq design matrix
#'
#' @param countfile Output from featureCounts
#' @param sampleData A data.frame with sample names and conditions (control/treatment)
#' @param design How the DEXSeq design matrix should be built. Defaults to `sample + exon + condition:exon`
#' @param flattenedfile Annotation file
#'
#' @export
#'
...
DEXSeqDataSetFromFeatureCounts <- function (countfile, sampleData,
  ...
  read.table(countfile,skip = 2) %>%
    dplyr::arrange(V1,V3,V4) %>%
    dplyr::select(-(V2:V6)) -> dcounts
